...
struct fitsCommentType {
  ...
  explicit fitsCommentType(char *v) { static_cast<void>(v); }

  explicit fitsCommentType(const char *v) { static_cast<void>(v); }
};
...
struct fitsHistoryType {
  ...
  explicit fitsHistoryType(char *v) { static_cast<void>(v); }

  explicit fitsHistoryType(const char *v) { static_cast<void>(v); }
};
 ...
 *
 * \tparam scalarT is the type
 * \retval int > 0 if a constant is defined in cfitsio corresponding to the
 * native type \retval -1 if not a defined type in cfitsio
 */
template <typename scalarT>
...
/** The argument is modified if the first and/or last non-whitespace character
 * is '
 *
...
/**
 * \param headStr is a c-string which must be 81 characters in length, including
 * the '\n' \param keyword is the keyword name \param value is the value string
 * \param comment is the comment string
 */
void fitsPopulateCard(char headStr[81], char *keyword, char *value,
                      char *comment);
...
template <>
int fits_write_key<char *>(fitsfile *fptr, char *keyword, void *value,
                           char *comment);

template <>
int fits_write_key<std::string>(fitsfile *fptr, char *keyword, void *value,
                                char *comment);
 ...
 */
template <>
int fits_write_key<bool>(fitsfile *fptr, char *keyword, void *value,
                         char *comment);

template <>
int fits_write_key<fitsUnknownType>(fitsfile *fptr, char *keyword, void *value,
                                    char *comment);
...
void fitsErrText(std::string &explan,  ///< [out] the explanatory message
                 const std::string &filename,  ///< [in] the FITS file's name
