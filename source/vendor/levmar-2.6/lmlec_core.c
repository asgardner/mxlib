...
static int LMLEC_ELIM(LM_REAL *A, LM_REAL *b, LM_REAL *c, LM_REAL *Y,
  ...
  /* first, copy R from the upper triangular part of a to the lower part of r
   * (thus transposing it). R is rank x rank */
  for (j = 0; j < rank; ++j) {
    for (i = 0; i <= j; ++i) r[j + i * rank] = a[i + j * tm];
    for (i = j + 1; i < rank; ++i) r[j + i * rank] = 0.0;  // upper part is zero
  }
  /* r now contains R^T */
  ...
  if (b && c) {
    /* compute c=Y*b */
    for (i = 0; i < tm; ++i) {
      for (j = 0, tmp = 0.0; j < rank; ++j) tmp += Y[i * rank + j] * b[j];
                  ...
                  */
{
  ...
  /* Z^T*(p-c) */
  for (i = 0; i < mm; ++i) {
    for (j = 0, tmp = 0.0; j < m; ++j) tmp += Z[j * mm + i] * p[j];
    pp[i] = tmp;
  }
