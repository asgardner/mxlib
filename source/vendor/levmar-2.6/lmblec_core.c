  ...
  for (i = nn, j = 0; i < n; ++i, ++j) {
    switch (typ[j]) {
      case __BC_INTERVAL__:
        ...
        /* out of interval */
        tmp = ub[j] - lb[j];
        tmp = LM_CNST(4.0) * (LM_CNST(2.0) * p[j] - (lb[j] + ub[j])) /
              (tmp * tmp);
        jac[i * m + j] = w[j] * tmp;
        break;

      case __BC_LOW__:                // (lb[j]<=p[j])? 0.0 : -1.0;
        if (lb[j] <= p[j]) continue;  // corresp. jac element already 0
      ...
      case __BC_HIGH__:               // (p[j]<=ub[j])? 0.0 : 1.0;
        if (p[j] <= ub[j]) continue;  // corresp. jac element already 0
                  ...
                  */
{
  ...
  if (!lb && !ub) {
    fprintf(stderr, RCAT(LCAT(LEVMAR_BLEC_DER,
                              "(): lower and upper bounds for box constraints "
                              "cannot be both NULL, use "),
  ...
  if (x) { /* nonzero x */
    ...
    for (i = 0; i < n; ++i) data.x[i] = x[i];
    for (i = n; i < n + m; ++i) data.x[i] = 0.0;
  } else
    data.x = NULL;
                  ...
                  */
{
  ...
  if (!lb && !ub) {
    fprintf(stderr, RCAT(LCAT(LEVMAR_BLEC_DIF,
                              "(): lower and upper bounds for box constraints "
                              "cannot be both NULL, use "),
