...
static void BOXPROJECT(LM_REAL *p, LM_REAL *lb, LM_REAL *ub, int m) {
  ...
  if (!lb) { /* no lower bounds */
    ...
    for (i = m; i-- > 0;) p[i] = __MEDIAN3(lb[i], p[i], ub[i]);
}
#undef __MEDIAN3

/* pointwise scaling of bounds with the mx1 vector scl. If div=1 scaling is
 * by 1./scl. Either lb, ub can be NULL. If not NULL, they are mx1 vectors
 */
static void BOXSCALE(LM_REAL *lb, LM_REAL *ub, LM_REAL *scl, int m, int div) {
  ...
  if (!lb) { /* no lower bounds */
    if (!ub) /* no upper bounds */
      ...
      if (div) {
        for (i = m; i-- > 0;)
          if (ub[i] != LM_REAL_MAX) ub[i] = ub[i] / scl[i];
      } else {
        for (i = m; i-- > 0;)
          if (ub[i] != LM_REAL_MAX) ub[i] = ub[i] * scl[i];
      }
    ...
    if (div) {
      for (i = m; i-- > 0;)
        if (lb[i] != LM_REAL_MIN) lb[i] = lb[i] / scl[i];
    } else {
      for (i = m; i-- > 0;)
        if (lb[i] != LM_REAL_MIN) lb[i] = lb[i] * scl[i];
    }
...
static void LNSRCH(int m, LM_REAL *x, LM_REAL f, LM_REAL *g, LM_REAL *p,
   ...
   *
   * Translated (with a few changes) from Schnabel, Koontz & Weiss uncmin.f,
   v1.3
   * Main changes include the addition of box projection and modification of the
   scaling
   ...
   *			    state is not modified in xlnsrch (but can be modified by
   func). *	iretcd  <--	 return code *	mxtake  <--	 boolean flag
   indicating step of maximum length used *	stepmx  --> maximum allowable
   step size *	steptl  --> relative step size at which successive iterates
  ...
  *iretcd = 2;
  ...
  for (j = _LSITMAX_; j-- > 0;) {
    for (i = m; i-- > 0;) xpls[i] = x[i] + lambda * p[i];
      ...
      *iretcd = 1;
      ...
      if (!LM_FINITE(fpls)) {
        ...
        if (firstback) { /*	first backtrack: quadratic fit */
          ...
          if (tlmbda > lambda * LM_CNST(.5)) tlmbda = lambda * LM_CNST(.5);
        }
        plmbda = lambda;
        pfpls = fpls;
 ...
 *
 * The algorithm implemented by this function employs projected gradient steps.
 * Since steepest descent is very sensitive to poor scaling, diagonal scaling
 * has been implemented through the dscl argument: Instead of minimizing f(p)
 * for p, f(D*q) is minimized for q=D^-1*p, D being a diagonal scaling matrix
 * whose diagonal equals dscl (see Nocedal-Wright p.27). dscl should contain
 * "typical" magnitudes for the parameters p. A NULL value for dscl implies no
 * scaling. i.e. D=I. To account for scaling, the code divides the starting
 * point and box bounds pointwise by dscl. Moreover, before calling func and
 * jacf the scaling has to be undone (by multiplying), as should be done with
 * the final point. Note also that jac_q=jac_p*D, where jac_q, jac_p are the
 * jacobians w.r.t. q & p, resp.
 */
...
#pragma GCC diagnostic ignored "-Wunused-but-set-variable"
  ...
  if (n < m) {
    fprintf(stderr,
            LCAT(LEVMAR_BC_DER,
                 "(): cannot solve a problem with fewer measurements [%d] than "
                 "unknowns [%d]\n"),
            n, m);
    return LM_ERROR;
  }
  ...
  if (dscl) { /* check that scaling consts are valid */
    for (i = m; i-- > 0;)
      if (dscl[i] <= 0.0) {
        fprintf(stderr,
                LCAT(LEVMAR_BC_DER,
                     "(): scaling constants should be positive (scale %d: %g "
                     "<= 0)\n"),
                i, dscl[i]);
        return LM_ERROR;
      }
  ...
  for (k = 0; k < itmax && !stop; ++k) {
    ...
    if (!dscl) {
      ...
      for (i = n; i-- > 0;) {
        ...
        jacim = jac + i * m;
        for (j = m; j-- > 0;) jacim[j] *= dscl[j];  // jac[i*m+j]*=dscl[j];
      }
    }
  ...
  printf("-- errors %.9g %0.9g, #active %d [%d]\n", jacTe_inf, p_eL2, numactive, j);
    ...
    while (1) {
      ...
      if (issolved) {
...
#endif
        /* the following test ensures that the computation of pDp_eL2 has not
         * overflowed. Such an overflow does no harm here, thus it is not
        ...
        /* note that if the LM step is not taken, code falls through to the LM
         * line search below */
      } else {
      ...
      if (jacTeDp <= -rho * pow(Dp_L2, LM_CNST(_POW_) / LM_CNST(2.0))) {
         ...
         * evaluations */
        {
          int mxtake, iretcd;
          LM_REAL stepmx,
              steptl = LM_CNST(1e3) * (LM_REAL)sqrt(LM_REAL_EPSILON);
        ...
        for (t = tini; t > tmin; t *= beta) {
          for (i = 0; i < m; ++i) pDp[i] = p[i] + t * Dp[i];
          BOXPROJECT(pDp, lb, ub, m); /* project to feasible set */
        ...
        /* Note that this point can also be reached via a goto when LNSRCH()
         * fails. */
      gradproj:

        /* jacTe has been negated above. Being a descent direction, it is next
         * used to make a projected gradient step
         */

        /* compute ||g|| */
        for (i = 0, tmp = 0.0; i < m; ++i) tmp += jacTe[i] * jacTe[i];
        ...
        for (t = (gprevtaken) ? t : t0; t > tming; t *= beta) {
          for (i = 0; i < m; ++i) pDp[i] = p[i] - t * jacTe[i];
          BOXPROJECT(pDp, lb, ub, m); /* project to feasible set */
          for (i = 0, Dp_L2 = 0.0; i < m; ++i) {
            Dp[i] = tmp = pDp[i] - p[i];
            Dp_L2 += tmp * tmp;
          ...
          /* compute ||g^T * Dp||. Note that if pDp has not been altered by
           * projection (i.e. BOXPROJECT), jacTeDp=-t*||g||^2
           */
          for (i = 0, jacTeDp = 0.0; i < m; ++i) jacTeDp += jacTe[i] * Dp[i];

          if (gprevtaken &&
              pDp_eL2 <= p_eL2 + LM_CNST(2.0) * LM_CNST(0.99999) *
                                     jacTeDp) { /* starting t too small */
            t = t0;
            gprevtaken = 0;
            continue;
          }
          // if(LM_CNST(0.5)*pDp_eL2<=LM_CNST(0.5)*p_eL2 + alpha*jacTeDp)
          // terminatePGLS;
          if (pDp_eL2 <= p_eL2 + LM_CNST(2.0) * alpha * jacTeDp)
            goto terminatePGLS;

          // if(pDp_eL2<=p_eL2 - LM_CNST(2.0)*alpha/t*Dp_L2) goto terminatePGLS;
...
breaknested: /* NOTE: this point is also reached via an explicit goto! */
  ...
  if (covar) {
    ...
    if (dscl) { /* correct for the scaling */
      for (i = m; i-- > 0;)
        for (j = m; j-- > 0;) covar[i * m + j] *= (dscl[i] * dscl[j]);
    }
  }
                  ...
                  */
{
  ...
  ret = LEVMAR_BC_DER(LMBC_DIF_FUNC, LMBC_DIF_JACF, p, x, m, n, lb, ub, dscl,
