...
namespace mx
{
   namespace improc
   {
      ...
      KLIPreduction<_realT, _derotFunctObj, _evCalcT>::KLIPreduction(const std::string &dir,
                                                                     const std::string &prefix,
                                                                     const std::string &ext) : ADIobservation<_realT, _derotFunctObj>(dir, prefix, ext)
      {
      }

      template <typename _realT, class _derotFunctObj, typename _evCalcT>
      KLIPreduction<_realT, _derotFunctObj, _evCalcT>::KLIPreduction(const std::string &fileListFile) : ADIobservation<_realT, _derotFunctObj>(fileListFile)
      {
      }
      ...
      KLIPreduction<_realT, _derotFunctObj, _evCalcT>::KLIPreduction(const std::string &dir,
                                                                     ...
                                                                     const std::string &RDIext) : ADIobservation<_realT, _derotFunctObj>(dir, prefix, ext, RDIdir, RDIprefix, RDIext)
      {
         std::cerr << "KLIP 6\n";
      }
      ...
      KLIPreduction<_realT, _derotFunctObj, _evCalcT>::KLIPreduction(const std::string &fileListFile,
                                                                     const std::string &RDIfileListFile) : ADIobservation<_realT, _derotFunctObj>(fileListFile, RDIfileListFile)
      {
      }
                                                                   ...
                                                                   std::vector<_realT> maxq)
      {
         ...
         for (unsigned int i = 1; i < m_Nmodes.size(); ++i)
         {
         ...
         for (unsigned int n = 0; n < m_Nmodes.size(); ++n)
         {
         ...
         for (unsigned int regno = 0; regno < minr.size(); ++regno)
         {
                         ...
                         eigenImage<int> &imsIncluded)
      {
         ...
         if (excludeMethod == HCI::excludePixel || excludeMethod == HCI::excludeAngle)
         {
            ...
            {
               if (fabs(math::angleDiff<math::radians>(derotF.derotAngle(j), derotF.derotAngle(imno))) <= dang)
         ...
         if (excludeMethodMax == HCI::excludePixel || excludeMethodMax == HCI::excludeAngle)
         {
            for (int j = 0; j < Nims; ++j)
            {
               if (fabs(math::angleDiff<math::radians>(derotF.derotAngle(j), derotF.derotAngle(imno))) > dangMax)
                  allidx[j].included = false;
            }
         }
...
#pragma omp for
            for (int imno = 0; imno < this->m_Nims; ++imno)
            {
               ...
               for (unsigned int mode_i = 0; mode_i < m_Nmodes.size(); ++mode_i)
               {
      ...
      int KLIPreduction<_realT, _derotFunctObj, _evCalcT>::finalProcess()
      {
         if (this->m_postMedSub)
         {
            ...
            for (unsigned int n = 0; n < this->m_psfsub.size(); ++n)
            {
#pragma omp parallel
               {
...
#pragma omp for
                  for (unsigned int i = 0; i < this->m_psfsub[n].planes(); ++i)
                  {
         ...
         if (this->m_doWriteFinim == true || this->m_doOutputPSFSub == true)
         {
            ...
            if (m_Nmodes.size() > 0)
            {
               for (unsigned int nm = 0; nm < m_Nmodes.size() - 1; ++nm)
                  str << m_Nmodes[nm] << ",";
            ...
            {
               str.str("");
               for (unsigned int nm = 0; nm < m_minr.size() - 1; ++nm)
                  str << m_minr[nm] << ",";
            ...
            {
               str.str("");
               for (unsigned int nm = 0; nm < m_maxr.size() - 1; ++nm)
                  str << m_maxr[nm] << ",";
            ...
            {
               str.str("");
               for (unsigned int nm = 0; nm < m_minq.size() - 1; ++nm)
                  str << m_minq[nm] << ",";
            ...
            {
               str.str("");
               for (unsigned int nm = 0; nm < m_maxq.size() - 1; ++nm)
                  str << m_maxq[nm] << ",";
